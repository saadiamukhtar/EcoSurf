...
function renderPage() {
	...
	if (avgDay) {
		...
		var flight = 14 * 1000;
		var trees = 21 * 1000;
		var iceMeltperKgCarbon= 0.01;
		if (year >= flight) {
			var fe = document.getElementById('flights');
			var ft = document.getElementById('trees');
			var ice= document.getElementById('ice');
			const iceMelt = year * iceMeltperKgCarbon;
			ice.innerHTML= `❄️  Oh! That's equivalent to melting ${iceMelt.toFixed(2)} cubic meter of glacier.`;
			fe.innerHTML = "🛫  Whoa! That's equivalent to " + (year / flight).toFixed(0) + " flights between Mumbai and Delhi. ";
